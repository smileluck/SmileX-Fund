...
import { UserHolding, Wallet, formatCurrency, formatPercentage } from '@/lib/dataService';
...
export default function WalletManager({
  ...
  // 过滤当前钱包的持仓并添加估值数据
  const currentWalletHoldings = useMemo(() => {
    if (currentWalletId === 'summary') {
  ...
  }, [userHoldings, funds, currentWalletId, fundType]);
  
  // 根据排序状态对持仓数据进行排序
  const sortedHoldings = useMemo(() => {
    return [...currentWalletHoldings].sort((a, b) => {
  ...
  }, [currentWalletHoldings, sortField, sortDirection]);
